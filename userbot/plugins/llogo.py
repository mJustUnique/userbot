...
async def Logo(legend):
    evxnt = await legend.edit("`Processing...`")
    text = legend.pattern_match.group(1)
    if not text:
        await evxnt.edit( "`Give Some Text To Make a Logo`")
        return
    fnt = await get_font_file(legend.client, "@FontsBin")
    if legend.reply_to_msg_id:
        rply = await legend.get_reply_message()
    ...
    await bot.send_file(legend.chat_id, file_name, caption=f"**Generated By :** {mention}")
    await evxnt.delete()
    try:
        os.remove(file_name)
        os.remove(fnt)
